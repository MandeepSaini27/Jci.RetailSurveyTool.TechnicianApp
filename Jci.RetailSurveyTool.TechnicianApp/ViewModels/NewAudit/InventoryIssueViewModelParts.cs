...
namespace Jci.RetailSurveyTool.TechnicianApp.ViewModels.NewAudit
{
    public class InventoryIssueViewModelParts : BaseViewModel //partial //StoreAreaDetailsViewModel //AuditStoreAreaListViewModel //NewAuditViewModel
    {
        ...
        public async Task LoadIssueTypes()
        {
            ...
            try
            {
                foreach (var it in await LocalAppDatabase.GetRawConnection().Table<IssueType>().Where(x => (x.ShowForDeactivation == SelectedStoreArea.DeactivationArea || x.ShowForSystem == SelectedStoreArea.PedestalArea) && (x.CustomerID == null || x.CustomerID == StartAudit.ID)).ToListAsync())
                {
                    lock (IssueTypes)
                    {
                        IssueTypes.Add(it);
        ...
        public async Task LoadIssueCategories()
        {
            ...
            try
            {
                var getItems = await LocalAppDatabase.GetRawConnection().Table<IssueCategory>().Where(x => x.IssueTypeID == SelectedIssueType.ID).ToListAsync();
