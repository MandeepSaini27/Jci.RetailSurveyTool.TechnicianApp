...
//            MessagingCenter.Subscribe<CustomerSelectionViewModel, Customer>(this, MessageNames.SelectedCustomerMessage, (customerSelectionViewModel, customer) => OnCustomerChanged(customer));
        ...
        private async void OnServiceCallLookUpCommand()
        {
            ...
            {
                CustomerManualSelectionReason = DEFAULT_MANUAL_REASON;
                await ShowAlert(CustomerManualSelectionReason, "Exception " + ex.Message, "Ok");
            }
        ...
        private async void OnStartAuditCommand()
        {
            ...
            {
                await ShowAlert("Error", "Please enter a WO# and do a search first", "Ok");
                return;
            }
            if (SelectedCustomer == null)
            {
                await ShowAlert("Error", "Please select a customer first", "Ok");
                return;
            }
            if (SelectedStoreType == null)
            {
                await ShowAlert("Error", "Please select a store type", "Ok");
                return;
            }
        ...
        private void OnLoadStoreTypesCommand()
        {
            ...
            try
            {
                ...
                foreach (var item in LocalAppDatabase.GetRawConnection().Table<StoreType>().Where(x => x.CustomerID == CustomerID).ToListAsync().Result)
                {
                    StoreTypes.Add(item);
                }
            }
