...
namespace Jci.RetailSurveyTool.TechnicianApp.Behaviors
{
    public class EventToCommandBehavior : BindableBehavior<View>
    {
        ...
        {
            if (sender.ToString() == "Xamarin.Forms.Picker") 
            {
    

                var _picker = (sender as Picker);
                if (_picker.ItemsSource.Count > 0) { 
                    // do nothing
                }
                else {
                    return;
            ...
            if (sender.ToString() == "Jci.RetailSurveyTool.TechnicianApp.Controls.BorderlessPicker")
            {


                var _picker = (sender as BorderlessPicker);
                if (_picker.ItemsSource.Count > 0)
                {
                    // do nothing
                }
